...
from method2 import fill_noSilence
from method3 import crop
...
class TestMethods(unittest.TestCase):
    ...
    def test_method2(self):        
        ...
        test_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "test")
        #creating an export path for testing purpose, gotta do it here, due to creating this one immedeately before method call, this can result in a race condition
        export_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "m2")
        try: 
            os.mkdir(test_path)
            os.mkdir(export_path)
        except OSError as error: 
        ...
        gen = Sine(440)
        short  = gen.to_audio_segment(duration=1000)
        short_with_pause = short + AudioSegment.silent(duration=500) + short
        medium = gen.to_audio_segment(duration=2000)
        medium_with_pause = medium + AudioSegment.silent(duration=500) + medium
        long = gen.to_audio_segment(duration=3000)
        long_with_pause = long + AudioSegment.silent(duration=500) + long
        
        #save test tones as files within the test folder
        short_with_pause.export(os.path.join(test_path, "short.wav"), format="wav")
        medium_with_pause.export(os.path.join(test_path, "medium.wav"), format="wav")
        long_with_pause.export(os.path.join(test_path, "long.wav"), format="wav")

        #sanity check: get duration of recently created sound files + opening soundfiles from folders works as intended
        self.assertAlmostEqual(short_with_pause.duration_seconds, 2.5, 3)
        self.assertAlmostEqual(AudioSegment.from_file(os.path.join(test_path,"long.wav")).duration_seconds, 6.5, 3)

        #testing method 2, minimal differences in length are most likely related to default params of the noise gate
        #method 2 consists of a combined application of method 2 & 1, for test of method 1 see above
        fill_noSilence(test_path, export_path)
        self.assertAlmostEqual(AudioSegment.from_file(os.path.join(export_path,"long_m2.wav")).duration_seconds, 6.0, 1)
        self.assertAlmostEqual(AudioSegment.from_file(os.path.join(export_path,"medium_m2.wav")).duration_seconds, 4.0, 1)
        self.assertAlmostEqual(AudioSegment.from_file(os.path.join(export_path,"short_m2.wav")).duration_seconds, 2.0, 1)
    ...
    def test_method3(self):        
        ...
        test_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "test")
        #creating an export path for testing purpose, gotta do it here, due to creating this one immedeately before method call, this can result in a race condition
        export_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "m3")
        try: 
            os.mkdir(test_path)
            os.mkdir(export_path)
        except OSError as error: 
        ...
        crop(test_path, export_path)
        self.assertEqual(AudioSegment.from_file(os.path.join(export_path,"long_m3.wav")).duration_seconds, expected)
        self.assertEqual(AudioSegment.from_file(os.path.join(export_path,"medium_m3.wav")).duration_seconds, expected)
        self.assertEqual(AudioSegment.from_file(os.path.join(export_path,"short_m3.wav")).duration_seconds, expected)
